...
```sh
python3.11 scripts/workflow.py apply --replace
```
...
codex login
claude auth login
python3.11 scripts/workflow.py configure-mcp
python3.11 scripts/workflow.py configure-mcp --execute
```
...
python3.11 scripts/workflow.py doctor --online
python3.11 scripts/macos/install-markdown-preview.py --set-default
```
...
```sh
python3.11 scripts/workflow.py restore
```
...
```sh
python3.11 scripts/workflow.py plan --target '/tmp/workflow demo' --local-only
python3.11 scripts/workflow.py apply --target '/tmp/workflow demo' --local-only
python3.11 scripts/workflow.py doctor --target '/tmp/workflow demo' --local-only --config-only
python3.11 scripts/workflow.py restore --target '/tmp/workflow demo'
```
